  ...
  return (
    <Drawer
        ...
        sx={{
          width: drawerWidth,
          flexShrink: 0,
          [`& .MuiDrawer-paper`]: { width: drawerWidth, boxSizing: 'border-box' ,backgroundColor : '#b3e5fc'},
        }}
        anchor="right"
      >
        <Toolbar />
...
<TextField fullWidth sx={{mx:0,my:1,} }
          id="email-search"
          label="Email"
          name="email"
          type="search"
    ...
    <Button  type="submit">Search</Button>
        ...
        {isSuccess&&data?.map(item=>(<SearchBarlist user={item} key={item.email} />))}
        </Box>
        </ThemeProvider>
      </Drawer>
  );
